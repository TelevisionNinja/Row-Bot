 ...
 * @param {*} alreadyMiss boolean for whether the interval was already detected as a miss
 */
function getMsUntilInterval(hour, minute, startingHour, startingMin, interval = 1440, alreadyMiss = false) {
    ...
    if (deltaTimeMins > 0) {
        ...
        if (alreadyMiss) {
            timeRemaining = calcDelta(newHr, newMin, nowHour, nowMin, nowSec, nowMs).ms + intervalMs;
        }
        else {
            miss = true;

            const {
                timeRemaining: timeRemainingMiss,
                isNow: isNowMiss
            } = getMsUntilInterval(newHr, newMin, startingHour, startingMin, interval, miss);
...
function calcDelta(hour, minute, nowHour, nowMin, nowSec, nowMs) {
    ...
    const deltaTimeMins = 60 * deltaHours + deltaMins;
...
function createInterval(func, interval, hour, minute, currentIntervalHr, currentIntervalMin) {
    let {
        ...
        isNow,
        miss
    } = getMsUntilInterval(currentIntervalHr, currentIntervalMin, hour, minute, interval);
...
function startIntervalFunc(func, interval, hour, minute, executeOnIntervalStart = false, executeNow = false) {
    if ((getMsUntilInterval(hour, minute, hour, minute, interval).isNow && executeOnIntervalStart) || executeNow) {
        func();
    }
