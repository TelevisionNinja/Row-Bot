module.exports = class Interval {
    ...
    #getMsUntilIntervalTime() {
        ...
        if (!deltaTime) {
            ...
            // overshoot
            deltaTime += (~~(-deltaTime / this.#interval) + 1) * this.#interval;
        }

        // added delay of 2 seconds
        this.#timeRemaining = 60000 * deltaTime - this.#currentMs + 2000;
    }
     ...
     */
    #customTimeout() {
        this.#timeoutTolerance /= 2;
        this.#timeoutID = setTimeout(() => {
            if (Date.now() - this.#timeoutStart < this.#timeRemaining) {
                this.#customTimeout();
            }
            else {
                this.#createInterval();
     ...
     */
    #createTimeout() {
        this.#timeoutTolerance = this.#timeRemaining;
 ...
 * @param {*} tolerance 
 */
function customTimeout(callback, time, start, tolerance) {
    tolerance /= 2;
    setTimeout(() => {
        if (Date.now() - start < time) {
            customTimeout(callback, time, start, tolerance);
        }
        else {
            callback();
        }
...
function createTimeout(callback, time) {
    customTimeout(callback, time, Date.now(), time);
}