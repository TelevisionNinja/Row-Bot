...
function getMsUntilInterval(hour, minute, interval = 1440) {
    ...
    const timeRemaining = (60 * deltaTime - nowTime.getSeconds()) * 1000 - nowTime.getMilliseconds();
 ...
 * @param {*} notFirstExecution boolean for whether the call is not the first one
 */
function createInterval(func, interval, hour, minute, currentIntervalHr, currentIntervalMin, notFirstExecution = false) {
    const {
        timeRemaining,
        undershoot
    } = getMsUntilInterval(currentIntervalHr, currentIntervalMin, interval);

    if (!undershoot && notFirstExecution) {
    ...
    setTimeout(() => {
        // reset timeout
        createInterval(func, interval, hour, minute, currentIntervalHr, currentIntervalMin, true);
    }, timeRemaining);
}
