 ...
 * if the current time is the current interval, the 'isNow' boolean will be true, and the time remaining is the provided interval
 ...
 */
function getMsUntilInterval(hour, minute, startingHour, startingMin, interval = 1440) {
    let isNow = false;
    let overshoot = false;
    ...
    const deltaTime = 60 * deltaHours + deltaMins;
    ...
    if (deltaTime > 0) {
        ...
        timeRemaining = getMsUntilInterval(newHr, newMin, startingHour, startingMin, interval).timeRemaining;
        overshoot = true;
    }
    else {
