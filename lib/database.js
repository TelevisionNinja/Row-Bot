...
export const tulps = {
    async set(user_id, username, avatar, start_bracket, end_bracket) {
        return await tulpDB.query(`
    ...
    async delete(user_id, username) {
        return await tulpDB.query(`
        DELETE FROM tulps
        WHERE user_id = $1 AND username = $2;
        `, [user_id, username]);
    },
    async updateBrackets(user_id, username, start_bracket, end_bracket) {
        return await tulpDB.query(`
        UPDATE tulps
    ...
    async updateUsername(user_id, old_username, new_username) {
        return await tulpDB.query(`
        UPDATE tulps
        SET username = $3
        WHERE user_id = $1 AND username = $2;
    ...
    async updateUsernameAndBrackets(user_id, old_username, new_username, old_start_bracket, new_start_bracket, end_bracket) {
        return await tulpDB.query(`
        UPDATE tulps
    ...
    async updateAvatar(user_id, username, avatar) {
        return await tulpDB.query(`
        UPDATE tulps
        SET avatar = $3
        WHERE user_id = '$1 AND username = $2;
