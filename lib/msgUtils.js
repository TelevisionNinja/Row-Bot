...
async function getWebhook(msg) {
    ...
    if (webhook) {
        return new WebhookClient(webhook.webhook.id, webhook.webhook.token);
    }
 ...
 * @param {*} webhook 
 */
async function tryWebhook(msgObj, msg, name, avatar, webhook) {
    try {
...
export async function sendWebhookMsg(msgObj, msg, name, avatar) {
    ...
    tryWebhook(msgObj, msg, name, avatar, webhook);
}
 ...
 * @param {*} webhook 
 */
export async function sendWebhookMsgUsingWebhook(msgObj, msg, name, avatar, webhook) {
    if (webhook) {
        const webhookObj = new WebhookClient(webhook.id, webhook.token);

        tryWebhook(msgObj, msg, name, avatar, webhookObj);
    }
    else {
