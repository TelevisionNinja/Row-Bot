...
const { tulpWebhooks } = require('./database.js');
 ...
 * @returns 
 */
async function sendAuthorDm(msg, content, file = []) {
    try {
        await msg.author.send(content, file);
...
function extractNameAndAvatar(msg, args) {
    ...
    if (success) {
        ...
        if (hasImgAttachment) {
            ...
            avatarLink = link;
            validURL = stringUtils.isValidURL(avatarLink);
        }
    }
 ...
 * @param {*} avatar 
 */
async function sendWebhookMsg(msgObj, msg, name, avatar) {
