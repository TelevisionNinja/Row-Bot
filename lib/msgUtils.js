 ...
 * @returns 
 */
export function sendTypingMsg(recipient, sendingMsg, readingMsg = '', isReply = false) {
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            setTimeout(async () => {
                try {
                    if (typeof recipient.channel === 'undefined') { // is channel
                        resolve(await recipient.send(sendingMsg));
                    }
                    else { // is message
                        if (isReply) {
                            resolve(await recipient.reply(sendingMsg));
                        }
                        else {
                            resolve(await recipient.channel.send(sendingMsg));
                        }
                    }
                }
                catch (error) {
