...
function extractNameAndAvatar(msg, args) {
    ...
    let imgAttachment = typeof attachment !== 'undefined';
    let imgLinkArg = argArr.length;

    if ((!imgAttachment && !imgLinkArg) || !extractedUsername.length) {
        fail = true;
    }
    else {
    ...
    if (stringUtils.isValidURL(avatarLink)) {
        validURL = true;
    }
