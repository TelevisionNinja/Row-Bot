...
function tagArrToParsedTagArr(tagArr, whitespace) {
    ...
    for (let i = 0, n = tagArr.length; i < n; i++) {
        ...
        const tag = replaceAll(tagArr[i].trim(), ' ', whitespace);
        if (tag !== '') {
            tagSet.add(tag);
        }
    }
...
/**
 * created a replaceAll function to use until the replaceAll native function is added
 * 
 * @param {*} str string
...
function replaceAll(str, subStr, replacement) {
    return str.replace(
        new RegExp(escapeRegex(subStr), 'g'),
        replacement
    );
}
