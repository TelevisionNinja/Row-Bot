 ...
 * @returns 
 */
 function cutOff(str, charLimit = 2000) {
 ...
 */
const isValidURLRegex = new RegExp(/^(https?:\/\/)(\w{0,}\w)(\.(\w{0,}\w))?\.(\w{1,}\w)/i);
 ...
 * check if the top level domain is of 2 or more characters
 */
const containsURLRegex = new RegExp(/(https?:\/\/)(\w{0,}\w)(\.(\w{0,}\w))?\.(\w{1,}\w)/i);
 ...
 * @returns 
 */
function includesPhrase(str, word, caseSensitive = true) {
    const escapedWord = `\\b${escapeRegex(word)}\\b`;
    const regex = caseSensitive ? new RegExp(escapedWord) : new RegExp(escapedWord, 'i');
